...
🟡 Phase 5   Docker ✅ · tags ✅ · pyramid ✅ · sharding ✅ · matrix ✅ · CD ✅ · visual ✅ · staging→prod gate ✅
```
...
App repo CI:    lint → unit → integration → publish-image (GHCR)
                → deploy STAGING → smoke staging → ⏸ approval → deploy PROD → smoke prod
```

The app pipeline runs the full CD shape **test → build → promote → verify**: on green
`main` the same commit is deployed to **staging**, smoke-tested, then — after a
**manual approval gate** (GitHub Environments) — promoted to **production** and
smoke-tested again. Live: [staging](https://demo-shop-app-staging.onrender.com) ·
[production](https://demo-shop-app-mlkv.onrender.com). See
[ADR-010](docs/adr/010-deploy-to-environment-and-post-deploy-smoke.md) and
[ADR-012](docs/adr/012-staging-production-promotion-gate.md).
